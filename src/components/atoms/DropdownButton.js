  ...
  return (
    <div className="relative w-full sm:w-1/3 md:w-1/3 lg:w-1/4 xl:w-1/4">
      <button
        ...
        className="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-lg focus:outline-none flex items-center justify-between w-full"
      >
      ...
      {dropdownOpen && (
        <div className="absolute right-0 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
          <button
            ...
            className={`w-full px-4 py-2 text-left ${infoType === 'domain' ? 'bg-blue-500 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}
          >
            Domain Info
          </button>
          <button
            onClick={() => { setInfoType('contact'); toggleDropdown(); }}
            className={`w-full px-4 py-2 text-left ${infoType === 'contact' ? 'bg-blue-500 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}
          >
            Contact Info
          </button>
          <button
            onClick={() => { setInfoType('both'); toggleDropdown(); }}
            className={`w-full px-4 py-2 text-left ${infoType === 'both' ? 'bg-blue-500 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}
          >
            Both
          </button>
        </div>
